...
### Figure 2 (profits) and 3(?): Declining surplus
phi1 = 0.8# share in surplus of first mover
phi2 = 0.9# share in surplus of first mover (2nd graph)
#
(t_phi.vec = t.vec)
...
#
## stopped here 2022_3_2. Make graphs with profits, show how fast they converge. 
## A graph showing surplus declining and how to compute the number of pregames needed. 
...
# finalizing Figure 2(?): profits in the general model
...
(n02.vec =  -ceiling(log(thresh02)/log(2*phi.vec-1)-1))
(n01.vec =  -ceiling(log(thresh01)/log(2*phi.vec-1)-1))
(n005.vec = -ceiling(log(thresh005)/log(2*phi.vec-1)-1))
(n001.vec = -ceiling(log(thresh001)/log(2*phi.vec-1)-1))

# finalizing table
(ngames.df = data.frame(phi.vec, phi_2nd.vec, n02.vec, n01.vec, n005.vec, n001.vec))
dim(ngames.df)
